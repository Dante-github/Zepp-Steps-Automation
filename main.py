...
def login(account, password):
    ...
    if re.match(PHONE_PATTERN, account):
        ...
        third_name = "email"   # 邮箱账号用 email 类型（原 "huami" 会导致 0106）
    ...
    res2_body = res2.text   # 先保留原始响应便于诊断
    if res2.status_code == 200:   # 检查状态码是否为200,是则请求成功
        ...
        try:
            ...
            print(f"错误:第二步登录响应中无 token_info,HTTP {r2sc},响应内容(前600字):{res2_body[:600]}")
            return None, None
    else:
        print(f"错误:登录失败,Login Code={res2.status_code},响应内容(前600字):{res2_body[:600]}")
        return None, None
...
if __name__ == "__main__":
    ...
    # 配置格式: [账号, 密码, 步数设置]
        ...
        # - None: 使用默认随机范围
    AccountGroup = [
        [306191453@qq.com, lixiaoyao617, [18000, 20000]],   # 随机范围 8000-10000（按需修改）
        [253659684@qq.com, mayday0923, [18000, 20000]],  # 使用随机范围20000-40000
